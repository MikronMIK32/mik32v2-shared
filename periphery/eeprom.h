...
/**
 * Исправление чтения по шине AHB после выполнения операций по шине APB.
 * 
 * Выполнение двух чтений из разных адресов по шине AHB после выполнения 
 * операций по шине APB гарантирует, что на выходе блока EEPROM устанавливаются 
 * корректные значения, а логика блока и шины готовы к дальшейшим чтениям 
 * по шине AHB и выборке инструкций.
 */
#define EEPROM_AHB_READ_FIX() asm volatile( \
    "lw x0,0(%0)\n\t"                         \
